...
`teaching_toolbox/` 提供本地网页版工作台，当前合并了学情反馈表、OJ/GESP 题库检索、课后服务话术、课前备课和 Agent 工作台入口。Agent 工作台先负责工具注册表和可追踪 run 计划，后续再接入专题题单生成、审计和课件/视频 workflow。
...
- 默认在当前课堂作业列表的前一个列表里匹配 `--after-class-problems`
- 不从当前训练里匹配课后题，避免课堂题和课后题混在同一来源
- 某个课后题在上一列表没匹配到时，会输出警告并跳过该题
